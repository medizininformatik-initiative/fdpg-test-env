...
resource "hcloud_primary_ip" "fdpg_dsf_fhir" {
  name          = terraform.workspace == "default" ? "fdpg-dsf-fhir" : format("fdpg-dsf-fhir-%s", terraform.workspace)
  type          = "ipv4"
  datacenter    = "fsn1-dc14"
  assignee_type = "server"
  auto_delete   = false
...
resource "hcloud_server" "fdpg-dsf-fhir" {
  name         = terraform.workspace == "default" ? "fdpg-dsf-fhir" : format("fdpg-dsf-fhir-%s", terraform.workspace)
  image        = "ubuntu-22.04"
  server_type  = "cx11"
  datacenter   = "fsn1-dc14"
  ...
  public_net {
    ipv4         = hcloud_primary_ip.fdpg_dsf_fhir.id
    ipv4_enabled = true
    ipv6_enabled = true
  }
}
